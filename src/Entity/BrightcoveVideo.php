...
class BrightcoveVideo extends BrightcoveVideoPlaylistCmsEntity implements BrightcoveVideoInterface {
  ...
  protected function getIngestionToken() {
    ...
    try {
      ...
      do {
        $token = Crypt::hmacBase64($this->getBrightcoveId(), Crypt::randomBytesBase64() . Settings::getHashSalt());
    ...
    if ($upload) {
      ...
      switch ($status) {
        case SAVED_NEW:
          ...
          $this->setBrightcoveId($saved_video->getId());
          $this->setCreatedTime(strtotime($saved_video->getCreatedAt()));
          break;

        case SAVED_UPDATED:
          // Set Video ID.
          $video->setId($this->getBrightcoveId());
      ...
      if (!is_null($this->ingestRequest)) {
        ...
        $di->createIngest($this->getBrightcoveId(), $this->ingestRequest);
      }
    }
  ...
  public function delete($local_only = FALSE) {
    ...
    if (!$this->isNew() && !$local_only) {
      ...
      try {
        $client->request('DELETE', 1, 'cms', $api_client->getAccountId(), "/videos/{$this->getBrightcoveId()}/references", NULL);

        // Delete video.
        $cms = BrightcoveUtil::getCmsApi($this->getApiClient());
        $cms->deleteVideo($this->getBrightcoveId());
